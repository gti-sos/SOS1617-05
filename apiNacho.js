...
exports.register = function(app, port, BASE_API_PATH, checkKey) {
    ...
    app.get("/usData", (req, res) => {
        ...
        var options = {
            ...
            host: 'zlzlap7j50.execute-api.us-east-1.amazonaws.com/prod',
            //path: '/prod'
        };
        callback = function(response) {
            var str = '';
    ...
    //This function loads fewer resources than original lwd (if the api contains no resources)
    app.get("/api/v2/elections-voting-stats/loadWholeData", function(request, response) {
        if (!checkKey(request, response)) {
            return;
        }
        console.log('INFO: Initialiting DB...');
